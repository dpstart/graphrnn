...
def sample_sigmoid(y, sample, thresh=0.5, sample_time=2, device="cpu"):
    ...
    if sample:
        if sample_time>1:
            y_result = torch.autograd.Variable(torch.rand(y.size(0),y.size(1),y.size(2))).to(device)
            # loop over all batches
            for i in range(y_result.size(0)):
                # do 'multi_sample' times sampling
                for j in range(sample_time):
                    y_thresh = torch.autograd.Variable(torch.rand(y.size(1), y.size(2))).to(device)
            ...
            y_thresh = torch.autograd.Variable(torch.rand(y.size(0),y.size(1),y.size(2))).to(device)
...
def train_mlp_epoch(epoch, args, rnn, output, data_loader,
                    ...
                    scheduler_rnn, scheduler_output, device):
    rnn.train()
    output.train()
    loss_sum = 0
    for batch_idx, data in enumerate(data_loader):
        ...
        # clean
        y_pred = torch.nn.utils.rnn.pack_padded_sequence(y_pred, y_len, batch_first=True)
...
def test_mlp_epoch(epoch, args, rnn, output, test_batch_size=16, save_histogram=False,sample_time=1, device="cpu"):
    ...
    y_pred = torch.autograd.Variable(torch.zeros(test_batch_size, max_num_node, args.max_prev_node)).to(device) # normalized prediction score
    y_pred_long = torch.autograd.Variable(torch.zeros(test_batch_size, max_num_node, args.max_prev_node)).to(device) # discrete prediction
    x_step = torch.autograd.Variable(torch.ones(test_batch_size,1,args.max_prev_node)).to(device)
        ...
        if epoch % args.epochs_test == 0 and epoch>=args.epochs_test_start:
            for sample_time in range(1,4):
               ...
               while len(G_pred)<args.test_total_size:
                   G_pred_step = test_mlp_epoch(epoch, args, rnn, output, test_batch_size=args.test_batch_size,sample_time=sample_time, device=device)
